...
class EdgeSwap(nn.Module):
    ...
    @staticmethod
    def edge_swap(bboxes: torch.Tensor, version: str|torch.Tensor) -> torch.Tensor:
        if type(version) != str:
            version = version_index_to_str(version)

        if version == 'le135':#硬编码
            return EdgeSwap._edge_swap_le135(bboxes)
        else:
            raise NotImplementedError(f"Unsupported version: {version}")

    @staticmethod
    def _edge_swap_le135(bboxes: torch.Tensor) -> torch.Tensor:
        """
        归一化旋转框到 le135 格式。
