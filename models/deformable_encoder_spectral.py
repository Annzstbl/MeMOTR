...
class DeformableEncoderSpectral(nn.Module):
    def __init__(self, encoder_layer, num_layers, use_checkpoint: bool):
        super(DeformableEncoderSpectral, self).__init__()
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, spectral=None):
        ...
        for _, layer in enumerate(self.layers):
            if self.use_checkpoint:
                ...
                def fn(x, i):
                    x = self.layers[i](x, pos, spectral, reference_points, spatial_shapes, level_start_index, padding_mask)
                    x = self.layers[i + 1](x, pos, spectral, reference_points, spatial_shapes, level_start_index, padding_mask)
                    x = self.layers[i + 2](x, pos, spectral, reference_points, spatial_shapes, level_start_index, padding_mask)
                    return x
                if _ % 3 == 0:
...
class DeformableEncoderLayerSpectral(nn.Module):
    """
    input:
    output: (B, Nq, C)
    """
