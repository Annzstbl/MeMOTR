...
from .deformable_encoder_20260310 import DeformableEncoderLayer20260310, DeformableEncoder20260310
from .deformable_decoder_20260310 import DeformableDecoderLayer, DeformableDecoder
from ..ops.modules import MSDeformAttn, MSDeformAttnSpectral, MSDeformAttn_Rotate
from ..mlp import MLP
from deprecated.sphinx import deprecated


class DeformableTransformer20260310(nn.Module):
    ...
    def forward(self, 
                ...
                prior_src_tokens: List[torch.Tensor],
                prior_spectral_tokens: List[torch.Tensor],
                prior_pos_embeddings: List[torch.Tensor],
                ):
        '''
            ...
            prior_src_tokens: lvl * [B, C, 2]
            prior_spectral_tokens: lvl * [B, 8, 2]
            prior_pos_embeddings: lvl * [B, C, 2]
        ...
        prior_num_tokens = []
        prior_src_tokens_flatten = []
        prior_spectral_tokens_flatten = []
        prior_lvl_pos_embeddings_flatten = []


        # 展平, 位置编码加上层级权重, encoder光谱权重映射为特征
        for lvl, (src, mask, pos_embed, spectral_weight, prior_src_token, prior_spectral_token, prior_pos_embedding) in enumerate(zip(srcs, masks, pos_embeds, spectral_weights, prior_src_tokens, prior_spectral_tokens, prior_pos_embeddings)):
            ...
            #* 处理prior tokens
            num_prior_token = prior_src_token.shape[-1] 
            prior_src_token = prior_src_token.transpose(1, 2) #(B, num_prior_token, C)
            #? 输入的是三维tensor，形式是否正确？ 
            #? 需要normalize
            prior_spectral_token = self.spectral_embed(prior_spectral_token.transpose(1, 2)) #(B, num_prior_token, C) 
            prior_pos_embedding = prior_pos_embedding.transpose(1, 2) #(B, num_prior_token, C)
            prior_lvl_pos_embedding = prior_pos_embedding + self.level_embed[lvl].view(1, 1, -1) #(B, num_prior_token, C)

            prior_num_tokens.append(num_prior_token)
            prior_src_tokens_flatten.append(prior_src_token)
            prior_spectral_tokens_flatten.append(prior_spectral_token)
            prior_lvl_pos_embeddings_flatten.append(prior_lvl_pos_embedding)
        ...
        prior_src_tokens_flatten = torch.cat(prior_src_tokens_flatten, 1) #(B, sum(num_prior_token), C)
        prior_spectral_tokens_flatten = torch.cat(prior_spectral_tokens_flatten, 1) #(B, sum(num_prior_token), C)
        prior_lvl_pos_embeddings_flatten = torch.cat(prior_lvl_pos_embeddings_flatten, 1) #(B, sum(num_prior_token), C)
        prior_nums = torch.as_tensor(prior_num_tokens, dtype=torch.long, device=src_flatten.device) #(n_levels, )
        priro_level_start_index = torch.cat((prior_nums.new_zeros((1,)),
                                       prior_nums.cumsum(0)[:-1]))                          # (n_levels, )
        ...
        if self.use_checkpoint and (self.checkpoint_level == 2 or self.checkpoint_level == 3):
            ...
            memory = checkpoint(self.encoder, src_flatten, spatial_shapes, level_start_index,
                                ...
                                prior_src_tokens=prior_src_tokens_flatten,
                                prior_spectral_tokens=prior_spectral_tokens_flatten,
                                prior_pos_embeddings=prior_lvl_pos_embeddings_flatten)
        else:
            memory = self.encoder(
                src=src_flatten, 
                ...
                prior_spectral_tokens=prior_spectral_tokens_flatten,
                prior_pos_embeddings=prior_lvl_pos_embeddings_flatten,
                priro_level_start_index = priro_level_start_index,
            )
        bs, _, c = memory.shape
...
# 向外暴露一个简短别名，便于与旧代码兼容
DeformableTransformer = DeformableTransformer20260310


def build(config: dict, rope_pos_module: Optional[nn.Module] = None):
    return DeformableTransformer20260310(
