...
class DeformableDecoderSpectral(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos, query_mask, src_padding_mask, query_spectral_weights):
        """
        Args:
            ...
            query_spectral_weights:       spectral weights, (B, Nq, C=8)
...
class DeformableDecoderLayerSpectral(nn.Module):
    def __init__(self,
        ...
        Args:
            ...
            visualize:
        """
        super(DeformableDecoderLayerSpectral, self).__init__()
        self.visualize = visualize
        self.n_det_queries = n_det_queries
    ...
    @staticmethod
    def with_spectral_embed(tensor, query_spectral):
    ...
    @deprecated(version="1.0", reason="do not use")
    ...
    def forward(self, tgt, query_pos, query_spectral, reference_points,
        ...
        if merge_det_track is False:
            ...
            reference_points = reference_points[:, :self.n_det_queries, :, :] # (B, Nq_det, src_layers, 2/4)
            query_mask = query_mask[:, :self.n_det_queries] # (B, Nq_det)
            query_spectral = query_spectral[:, :self.n_det_queries, :] # (B, Nq_det, C)



        if self.extra_track_attn:# do not use
            tgt = self.forward_track_attn(tgt=tgt, query_pos=query_pos, query_mask=query_mask)

        tgt = self.forward_self_attn(tgt=tgt, query_pos=query_pos, query_spectral=query_spectral, query_mask=query_mask)
        # Cross Attention
