...
class DeformableTransformer(nn.Module):
    ...
    def _build_query_spectral_state(
        ...
        """
        # Spectral prior is kept independent from query content.
    ...
    def forward(self, srcs: List[torch.Tensor], masks: List[torch.Tensor], pos_embeds: Optional[List[torch.Tensor]],
        ...
        layer_output_queries, layer_output_refs, layer_input_queries, layer_cls_q_specs, layer_obs_q_specs, layer_track_obs_q_specs = self.decoder(
            ...
            q_spec=None,
            spectral_weights=spectral_weights,
            q_spec_builder=self._build_query_spectral_state_spec_only_rbox if self.use_q_spec else None,
            track_q_spec=track_q_spec if self.use_q_spec else None,
        )
        init_q_spec = None
        obs_q_spec = None
        ...
        if self.use_q_spec and layer_obs_q_specs is not None:
            ...
            #   det part   -> final dynamic det observation
            #   track part -> final dynamic track observation
            obs_q_spec = layer_obs_q_specs[-1]
