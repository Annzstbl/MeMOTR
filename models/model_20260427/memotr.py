...
class MeMOTR(nn.Module):
    def __init__(self, backbone: BackboneWithPE, transformer: DeformableTransformer,
        ...
        self.det_q_spec_cls_head = nn.Linear(in_features=self.hidden_dim, out_features=num_classes) if self.use_q_spec else None
        self.track_q_spec_cls_head = nn.Linear(in_features=2 * self.hidden_dim, out_features=num_classes) if self.use_q_spec else None
        self.det_q_spec_cls_alpha = nn.Parameter(torch.tensor(0.1)) if self.use_q_spec else None
        self.track_q_spec_cls_alpha = nn.Parameter(torch.tensor(0.1)) if self.use_q_spec else None
        ...
        if self.det_q_spec_cls_head is not None:
            nn.init.zeros_(self.det_q_spec_cls_head.weight)
            nn.init.zeros_(self.det_q_spec_cls_head.bias)
        if self.track_q_spec_cls_head is not None:
            nn.init.zeros_(self.track_q_spec_cls_head.weight)
            nn.init.zeros_(self.track_q_spec_cls_head.bias)
        ...
        if self.with_box_refine:
            ...
            if self.det_q_spec_cls_head is not None:
                self.det_q_spec_cls_head = get_clones(self.det_q_spec_cls_head, self.transformer.get_n_dec_layers())
            if self.track_q_spec_cls_head is not None:
                self.track_q_spec_cls_head = get_clones(self.track_q_spec_cls_head, self.transformer.get_n_dec_layers())
    ...
    def forward(self, frame: NestedTensor, tracks: List[TrackInstances],
        ...
        outputs, init_reference, inter_references, inter_queries, init_q_spec, obs_q_spec, layer_cls_q_specs, layer_track_obs_q_specs = self.transformer(
            srcs=srcs,
            masks=masks,
            pos_embeds=pos,
        ...
        for level in range(outputs.shape[0]):
            ...
            if (
                self.use_q_spec
                and layer_cls_q_specs is not None
                and self.det_q_spec_cls_head is not None
                and self.track_q_spec_cls_head is not None
            ):
                layer_q_spec = layer_cls_q_specs[level]
                det_count = min(self.n_det_queries, layer_q_spec.shape[1], output_class.shape[1])
                if det_count > 0:
                    det_q_spec = layer_q_spec[:, :det_count, :]
                    det_delta_logits = self.det_q_spec_cls_head[level](det_q_spec)
                    alpha_det = torch.sigmoid(self.det_q_spec_cls_alpha)
                    output_class[:, :det_count, :] = output_class[:, :det_count, :] + alpha_det * det_delta_logits

                if layer_q_spec.shape[1] > self.n_det_queries and output_class.shape[1] > self.n_det_queries:
                    track_count = min(layer_q_spec.shape[1], output_class.shape[1]) - self.n_det_queries
                    if track_count > 0 and layer_track_obs_q_specs is not None:
                        track_mem_q_spec = layer_q_spec[:, self.n_det_queries:self.n_det_queries + track_count, :]
                        track_obs_q_spec = layer_track_obs_q_specs[level]
                        assert track_obs_q_spec.shape[1] == track_count, \
                            f"track obs q_spec count mismatch: expect {track_count}, got {track_obs_q_spec.shape[1]}."
                        track_pair_feat = torch.cat([
                            track_mem_q_spec,
                            track_obs_q_spec,
                        ], dim=-1)
                        track_delta_logits = self.track_q_spec_cls_head[level](track_pair_feat)
                        alpha_track = torch.sigmoid(self.track_q_spec_cls_alpha)
                        output_class[:, self.n_det_queries:self.n_det_queries + track_count, :] = \
                            output_class[:, self.n_det_queries:self.n_det_queries + track_count, :] + \
                            alpha_track * track_delta_logits
        ...
        if init_q_spec is not None:
            # Preferred naming: align with last_layer_input_* style.
            # Kept old key for compatibility.
            res["last_layer_input_q_spec"] = init_q_spec
            res["init_q_spec"] = init_q_spec
        if obs_q_spec is not None:
            # Preferred naming: align with last_layer_output_* style.
            # Kept old key for compatibility.
            res["last_layer_output_q_spec"] = obs_q_spec
            res["obs_q_spec"] = obs_q_spec
        if debug:
