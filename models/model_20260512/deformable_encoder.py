    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None,
        ...
        for idx, layer in enumerate(self.layers):
            if self.use_checkpoint:
                ...
                def fn(x, add_tokens_inner, idx=idx):
                    x, add_tokens_inner = self.layers[idx](x, pos, spectral, reference_points, spatial_shapes, level_start_index,
                                                           padding_mask, add_tokens_inner, add_specs, add_pos_embeds, add_level_start_index)
                    x, add_tokens_inner = self.layers[idx + 1](x, pos, spectral, reference_points, spatial_shapes, level_start_index,
                                                               padding_mask, add_tokens_inner, add_specs, add_pos_embeds, add_level_start_index)
                    x, add_tokens_inner = self.layers[idx + 2](x, pos, spectral, reference_points, spatial_shapes, level_start_index,
                                                               padding_mask, add_tokens_inner, add_specs, add_pos_embeds, add_level_start_index)
                    return x, add_tokens_inner

                if idx % 3 == 0:
                    output, add_tokens = checkpoint(fn, output, add_tokens, use_reentrant=False)
            else:
