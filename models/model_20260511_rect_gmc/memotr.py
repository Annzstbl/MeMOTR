...
from hsmot.mmlab.hs_rectmot import denormalize_cxcywh, normalize_cxcywh
from structures.track_instances import TrackInstances
from utils.GMC import compensate_rect_boxes
...
class MeMOTR(nn.Module):
    ...
    def __init__(self, backbone: BackboneWithPE, transformer: DeformableTransformer,
        ...
        if self.use_dab:
            self.det_anchor = nn.Parameter(torch.randn(self.n_det_queries, self.BBOX_DIM))
    ...
    def apply_gmc_to_track_refs(self, tracks: List[TrackInstances],
        ...
        """Warp existing track ref_pts (cxcywh) with GMC before decoder (inference only)."""
        ...
        for b, track in enumerate(tracks):
            ...
            if ref_pts.shape[-1] != self.BBOX_DIM:
                raise ValueError(
                    f"GMC ref update expects {self.BBOX_DIM}D ref_pts, got shape {ref_pts.shape}."
                )

            norm_boxes = ref_pts.sigmoid().detach()
            boxes_pix = denormalize_cxcywh(norm_boxes, (eff_h, eff_w))
            ...
            boxes_comp = torch.from_numpy(
                compensate_rect_boxes(boxes_pix.cpu().numpy(), gmc_matrix_np)
            ).to(device=ref_pts.device, dtype=ref_pts.dtype)
            norm_comp = normalize_cxcywh(boxes_comp, (eff_h, eff_w))
    ...
    def get_track_reference_points(self, tracks: List[TrackInstances]) -> torch.Tensor:
        ...
        references = torch.zeros((len(tracks), max_len, self.BBOX_DIM))
    ...
    def get_prior_map(self, tracks: List[TrackInstances], gmcs: torch.Tensor, frame: NestedTensor) -> torch.Tensor:
        ...
        for b in range(batch_size):
            ...
            theta = torch.zeros((boxes.shape[0], 1), device=device, dtype=boxes.dtype)
            boxes = torch.cat((boxes, theta), dim=-1)
...
def build(config: dict) -> MeMOTR:
    ...
    scem = build_scem(config={**config["SCEM"], "INPUT_CHANNELS": config["INPUT_CHANNELS"]})
