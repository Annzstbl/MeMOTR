...
from .deformable_decoder import DeformableDecoder, DeformableDecoderLayer, MSDeformAttnQSpec
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, n_feature_levels=4, n_heads=8, n_enc_points=4, n_dec_points=4,
                 ...
                 use_q_spec: bool = True, input_spec_channels: int = 8):
        super().__init__()
        ...
        decoder_layer = DeformableDecoderLayer(d_model=d_model, d_ffn=d_ffn, dropout=dropout, activation=activation,
                                               ...
                                               visualize=self.visualize, use_q_spec=use_q_spec)
        ...
        self.spectral_embed = MLP(
            input_dim=input_spec_channels, hidden_dim=self.d_model, output_dim=self.d_model, num_layers=2,
        )
        self.q_spec_obs_embed = MLP(
            input_dim=input_spec_channels * n_feature_levels,
            hidden_dim=self.d_model, output_dim=self.d_model, num_layers=2,
        )
    ...
    def reset_parameters(self): 
        ...
        for module in self.modules():
            if any([
                ...
                isinstance(module, MSDeformAttnQSpec),
    ...
    def _build_query_spectral_state_spec_tgt_fusion_center(
        self,
        spectral_weights: List[torch.Tensor],
        reference_points: torch.Tensor,
        tgt: torch.Tensor,
    ...
    def _build_query_spectral_state(
        ...
        return self._build_query_spectral_state_spec_tgt_fusion_center(
            spectral_weights=spectral_weights,
            reference_points=reference_points,
            tgt=tgt,
            valid_ratios=valid_ratios,
    ...
    def _build_decoder_input_q_spec(
        self,
        spectral_weights: List[torch.Tensor],
        reference_points: torch.Tensor,
        tgt: torch.Tensor,
        valid_ratios: torch.Tensor,
        track_q_spec: Optional[torch.Tensor],
    ) -> torch.Tensor:
        det_q_spec = self._build_query_spectral_state(
            spectral_weights=spectral_weights,
            reference_points=reference_points[:, :self.n_det_queries],
            tgt=tgt[:, :self.n_det_queries],
            valid_ratios=valid_ratios,
        )
        num_tracks = reference_points.shape[1] - self.n_det_queries
        if num_tracks <= 0:
            return det_q_spec

        assert track_q_spec.shape[1] == num_tracks, f"track_q_spec shape mismatch: expect {num_tracks} tracks, got {track_q_spec.shape[1]}."
        assert track_q_spec.device == det_q_spec.device, f"track_q_spec device mismatch: expect {det_q_spec.device}, got {track_q_spec.device}."

        return torch.cat((det_q_spec, track_q_spec), dim=1)
    ...
    def forward(self, srcs: List[torch.Tensor], masks: List[torch.Tensor], pos_embeds: Optional[List[torch.Tensor]],
                ...
                additional_tokens: List[torch.Tensor], additional_specs: List[torch.Tensor],
                additional_pos_embeds: List[torch.Tensor], track_q_spec: Optional[torch.Tensor] = None):
        ...
        q_spec = None
        if self.use_q_spec:
            q_spec = self._build_decoder_input_q_spec(
                spectral_weights=spectral_weights,
        ...
        obs_q_spec = None
        if self.use_q_spec:
            # 根据预测的框位置和output queries 构建q_spec
            obs_q_spec = self._build_query_spectral_state(
                spectral_weights=spectral_weights,
                reference_points=layer_output_refs[-1],
                tgt=layer_output_queries[-1],
                valid_ratios=valid_ratios,
            )

        # - q_spec : (B, Q, C) = cat( new det spec , track_spec)
        # - obs_q_spec : (B, Q, C) = 根据这轮预测生成的q_spec


        return layer_output_queries, init_reference_points, layer_output_refs, layer_input_queries, q_spec, obs_q_spec
...
def build(config: dict, rope_pos_module: Optional[nn.Module] = None):
    return DeformableTransformer(
        ...
        use_q_spec=config.get("USE_Q_SPEC", False),
        input_spec_channels=config["INPUT_CHANNELS"],
    )
