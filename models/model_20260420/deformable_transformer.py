...
class DeformableTransformer(nn.Module):
    ...
    def _build_query_spectral_state(
        ...
        # q_spec is a query-aligned spectral prior initialized from dense spectral evidence and reference points.
        query_centers = reference_points[..., :2].detach()   # [B, Q, 2], optional but recommended for stability

        sampled_specs = []
        for lvl, lvl_spectral_weight in enumerate(spectral_weights):
            lvl_centers = query_centers * valid_ratios[:, None, lvl, :]   # align with valid region
            lvl_grid = (lvl_centers * 2.0 - 1.0).clamp(min=-1.0 + 1e-6, max=1.0 - 1e-6)
            lvl_grid = lvl_grid.unsqueeze(2)  # [B, Q, 1, 2]

            lvl_sampled = F.grid_sample(
                input=lvl_spectral_weight,
                grid=lvl_grid,
                mode="bilinear",
                padding_mode="zeros",
                align_corners=False,
        ...
        # better than raw mean: concat multi-level spectral observations
        pooled_spec = torch.cat(sampled_specs, dim=-1)  # [B, Q, 8 * L]
        pooled_spec_embed = self.q_spec_obs_embed(pooled_spec)

        q_spec0 = self.q_spec_fuse(torch.cat([pooled_spec_embed, tgt], dim=-1))
        q_spec0 = self.q_spec_norm(q_spec0)
        return q_spec0
