...
from utils.img_metas_utils import get_img_shape
...
class QueryUpdater(nn.Module):
    def __init__(self, hidden_dim: int, ffn_dim: int,
        ...
        # 正框 4D：pos_to_pos_embed -> 4 * (hidden_dim//2) = 2*hidden_dim（无 θ 的 +2）
    ...
    def _get_img_area(img_metas, batch_idx: int = 0) -> float:
        h_img, w_img = get_img_shape(img_metas, batch_idx=batch_idx)
        return h_img * w_img
