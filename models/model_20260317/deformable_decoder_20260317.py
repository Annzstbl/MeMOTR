...
class DeformableDecoder20260317(nn.Module):
    def __init__(
        self,
        decoder_layer,
        num_layers,
            ...
            if reference_points.shape[-1] == 5:
                reference_points_input = reference_points[:, :, None] * torch.cat(
                    [
                        src_valid_ratios,
                        src_valid_ratios,
                        torch.ones((*src_valid_ratios.shape[:-1], 1), device=src_valid_ratios.device),
                    ],
                    -1,
                )[:, None]
            else:
                ...
                if lid < self.merge_det_track_layer:
                    if self.use_dab:
                        reference_points = torch.cat(
                            (
                                new_reference_points[:, : self.n_det_queries, :].detach(),
...
class DeformableDecoderLayer20260317(nn.Module):
    def __init__(
        self,
        d_model=256,
        d_ffn=1024,
    ...
    def forward_self_attn(self, tgt, query_pos, query_mask):
        ...
        if self.visualize:
            torch.save(_[0, :, :].cpu(), "./outputs/visualize_tmp/decoder/attn_score.tensor")
