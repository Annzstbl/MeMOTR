...
from .figure_save import save_figure_outputs
...
class MeMOTR(nn.Module):
    ...
    def forward(self, frame: NestedTensor, tracks: List[TrackInstances],
                ...
                gmc: Optional[torch.Tensor] = None, figure: Optional[FigureContext] = None):
        need_debug = debug or (figure is not None and figure.enabled)
        capture_cross_attn = bool(
            figure is not None and figure.enabled and figure.save_cross_attn and figure.should_save_track()
        )
        ...
        outputs, init_reference, inter_references, inter_queries, init_q_spec, obs_q_spec, init_pooled_spec_rbox_levels, obs_pooled_spec_rbox_levels = self.transformer(
            srcs=srcs,
            masks=masks,
            pos_embeds=pos,
        ...
        if init_pooled_spec_rbox_levels is not None:
            res["init_pooled_spec_rbox_levels"] = init_pooled_spec_rbox_levels
        if obs_pooled_spec_rbox_levels is not None:
            res["obs_pooled_spec_rbox_levels"] = obs_pooled_spec_rbox_levels
        if need_debug:
        ...
        if figure is not None and figure.enabled:
            cross_attn_debug = self.transformer.pop_cross_attn_debug() if capture_cross_attn else None
            save_figure_outputs(
                res=res,
                figure=figure,
                tracks=tracks,
                cross_attn_debug=cross_attn_debug,
                n_feature_levels=self.n_feature_levels,
            )
        return res
