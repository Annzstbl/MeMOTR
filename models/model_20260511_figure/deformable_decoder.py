...
from ..mlp import MLP
from .cross_attn_figure import MSDeformAttnFigure as MSDeformAttn
...
class DeformableDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos, query_mask, src_padding_mask, q_spec=None, capture_cross_attn: bool = False):
        output = tgt
        if capture_cross_attn:
            self._cross_attn_debug = {}
        else:
            self._cross_attn_debug = None
        # 统一语义:
        ...
        for lid, layer in enumerate(self.layers):
            ...
            use_layer_checkpoint = self.use_checkpoint and not capture_cross_attn
            if use_layer_checkpoint:
                ...
                output = layer(
                    ...
                    query_mask=query_mask, src_padding_mask=src_padding_mask,
                    merge_det_track=(lid >= self.merge_det_track_layer),
                    q_spec=q_spec, capture_cross_attn=capture_cross_attn,
                )
                if capture_cross_attn:
                    debug = layer.cross_attn.pop_cross_attn_debug()
                    if debug is not None:
                        self._cross_attn_debug[lid] = {
                            "attention_weights": debug["attention_weights"].cpu(),
                            "sampling_locations": debug["sampling_locations"].cpu(),
                            "spatial_shapes": debug["spatial_shapes"].cpu(),
                        }
    ...
    def pop_cross_attn_debug(self):
        debug = getattr(self, "_cross_attn_debug", None)
        self._cross_attn_debug = None
        return debug or {}
...
class DeformableDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, dropout=0.1, activation="ReLU",
                 ...
                 n_det_queries=300, visualize: bool = False):
        super().__init__()
        self.visualize = visualize
        self.n_det_queries = n_det_queries
        ...
        self.cross_attn = MSDeformAttn(d_model=d_model, n_levels=n_levels, n_heads=n_heads, n_points=n_points, sigmoid_attn=sigmoid_attn)
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, query_mask,
                src_padding_mask=None, merge_det_track=False, q_spec=None, capture_cross_attn: bool = False):
        ...
        tgt2 = self.cross_attn(
            ...
            q_spec=q_spec,
            capture_debug=capture_cross_attn,
        )
