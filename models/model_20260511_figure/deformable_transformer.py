...
class DeformableTransformer(nn.Module):
    ...
    def _sample_query_spectral_obs_center(
        ...
        return pooled_spec, sampled_specs
    ...
    def _build_query_spectral_state_spec_only_rbox(
        ...
        pooled_spec, _ = self._sample_query_spectral_obs_rbox(
            spectral_weights=spectral_weights,
            reference_points=reference_points,
            valid_ratios=valid_ratios,
        )
    ...
    def forward(self, srcs: List[torch.Tensor], masks: List[torch.Tensor], pos_embeds: Optional[List[torch.Tensor]],
        ...
        obs_q_spec = None
        init_pooled_spec_rbox_levels = None
        obs_pooled_spec_rbox_levels = None
        if self.use_q_spec:
            ...
            _, init_pooled_spec_rbox_levels = self._sample_query_spectral_obs_rbox(
                spectral_weights=spectral_weights,
                reference_points=init_reference_points,
                valid_ratios=valid_ratios,
            )
            _, obs_pooled_spec_rbox_levels = self._sample_query_spectral_obs_rbox(
                spectral_weights=spectral_weights,
                reference_points=layer_output_refs[-1],
                valid_ratios=valid_ratios,
            )
        ...
        return (
            ...
            q_spec,
            obs_q_spec,
            init_pooled_spec_rbox_levels,
            obs_pooled_spec_rbox_levels,
        )
    ...
    def pop_cross_attn_debug(self):
        return self.decoder.pop_cross_attn_debug()
