...
class SCEM(nn.Module):
    def __init__(self, config):
        ...
        self.evidence_relation_net = nn.Sequential(
                nn.Conv2d(self.spectral_databse_num, self.evi_hidden_dim, 1, bias=False),
        ...
        self.evidence_weight_head = nn.ModuleList([
            nn.Conv2d(self.evi_hidden_dim, num_tokens * self.spectral_databse_num, kernel_size=1, bias=True)
            for num_tokens in [8, 4, 2, 1]
        ])
        self.evidence_temperature = 1
    ...
    def forward(
        ...
        for i, (feat, mask, token_num) in enumerate(zip(features, masks, token_nums)):
            ...
            relation_feat = self.evidence_relation_net(spectral_evidence_multilevel[i])
            evidence_weights_logits = self.evidence_weight_head[i](relation_feat)
            evidence_weights_logits = evidence_weights_logits.view(B, token_num, self.spectral_databse_num, H_i, W_i)
            evidence_weights_logits = evidence_weights_logits.masked_fill(mask.unsqueeze(1).unsqueeze(2), float("-inf"))
            evidence_weights = torch.softmax(
                evidence_weights_logits/self.evidence_temperature, dim=2
            )#[B, T, K, H, W] K做softmax
            evidence_weights = evidence_weights.masked_fill(mask.unsqueeze(1).unsqueeze(2), 0.0)
            ...
            tokens = torch.einsum("bts,bcs->btc", pool_weights_flat, feat_flat)# [B, T, C]

            beta = evidence_weights * pool_weights.unsqueeze(2)#[B, T, K, H, W]
            beta = beta.flatten(3).sum(dim=-1)#[B, T, K]
            beta = beta/ beta.sum(dim=-1, keepdim=True).clamp_min(1e-6) # [B, T, K]
