...
def _save_scem_token_debug_maps(token_debug: Dict[str, Any], map_out_dir: str, seq: str):
    ...
    for src_lvl, (ew_lvl, pw_lvl) in enumerate(zip(evidence_weights_levels, pool_weights_levels)):
        if not (torch.is_tensor(ew_lvl) and torch.is_tensor(pw_lvl)):
            continue
        # ew_lvl: [B,T,K,H,W], pw_lvl: [B,T,H,W]
        if ew_lvl.dim() != 5 or pw_lvl.dim() != 4:
            continue
        num_tokens = min(ew_lvl.shape[1], pw_lvl.shape[1])
        for tok_idx in range(num_tokens):
            ew_tok = ew_lvl[0, tok_idx]     # [K,H,W]
            pw_tok = pw_lvl[0, tok_idx]     # [H,W]

            # evidence_weights 是 K 维分布，先对 K 求均值得到单张 2D 图
            ew_mean = ew_tok.mean(dim=0).detach().cpu().numpy()
            ...
            db.save_scalar_heatmap(ew_mean, ew_path, vmin=float(ew_mean.min()), vmax=float(ew_mean.max()), cmap="jet")
...
def main():
    ...
    parser.add_argument(
        "--train-config",
        type=str,
        default="/data/users/litianhao/hsmot_code/MeMOTR/debug/20260330_197/20260330.yaml",
        help=(
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="/data4/litianhao/hsmot/memotr/spectralemb/autodl_20260331/checkpoint_39.pth",
        help=(
            "Checkpoint (.pth) path. "
            "Absolute path is used directly; "
