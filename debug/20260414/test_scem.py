...
def _resolve_train_config_path(train_config_arg: str) -> str:
    """
    Resolve train config path:
    - absolute path: use as-is
    - relative path: resolve relative to this script file
    """
...
def _resolve_checkpoint_path(checkpoint_arg: str, train_config: dict, train_cfg_path: str) -> str:
    ...
    - absolute path: use as-is
    - relative path: resolve under OUTPUTS_DIR in train config
    """
...
def _save_scem_token_debug_maps(
    ...
    for src_lvl, (ew_lvl, pw_lvl) in enumerate(zip(evidence_weights_levels, pool_weights_levels)):
        if not torch.is_tensor(pw_lvl) or pw_lvl.dim() != 4:
            continue
        # 兼容两种格式：
        # 1) 旧版 ew_lvl: Tensor [B,T,K,H,W]
        # 2) 新版 ew_lvl: {"a_pos": Tensor[B,T,K,H,W], "a_neg": Tensor[B,T,K,H,W]}
        ew_pos = None
        ew_neg = None
        if torch.is_tensor(ew_lvl):
            ...
            a_pos = ew_lvl.get("a_pos", None)
            a_neg = ew_lvl.get("a_neg", None)
        ...
        for tok_idx in range(num_tokens):
            ...
            pw_tok = pw_lvl[0, tok_idx]     # [H,W]

            # evidence_weights 是 K 维分布，先对 K 求均值得到单张 2D 图
            ew_pos_mean = ew_tok_pos.mean(dim=0).detach().cpu().numpy()
            pw_map = pw_tok.detach().cpu().numpy()

            ew_path = os.path.join(
                map_out_dir,
                f"{seq}__scem_tokdbg_srcL{src_lvl}_tok{tok_idx}__evidenceW_mean.png",
            )
            ew_neg_path = os.path.join(
                map_out_dir,
                f"{seq}__scem_tokdbg_srcL{src_lvl}_tok{tok_idx}__evidenceW_neg_mean.png",
            )
            pw_path = os.path.join(
                map_out_dir,
                f"{seq}__scem_tokdbg_srcL{src_lvl}_tok{tok_idx}__poolW.png",
            )
            pl_path = os.path.join(
                map_out_dir,
                f"{seq}__scem_tokdbg_srcL{src_lvl}_tok{tok_idx}__poolLogits.png",
            )
    ...
    for src_lvl, token_list in enumerate(corr_levels):
        if not isinstance(token_list, (list, tuple)):
            continue
        for tok_idx, per_tgt_levels in enumerate(token_list):
            ...
            for tgt_lvl, corr_map in enumerate(per_tgt_levels):
                if not torch.is_tensor(corr_map):
                    continue
                if corr_map.dim() == 4:
                    arr2d = corr_map[0, 0].detach().cpu().numpy()
                elif corr_map.dim() == 3:
                    arr2d = corr_map[0].detach().cpu().numpy()
                else:
                    continue
                last_tgt_lvl = tgt_lvl
    ...
    # 所有 level 的真实 token（来自 SCEM debug）一起计算两两相关性矩阵，只画一张图
    all_token_vecs: List[np.ndarray] = []
    all_token_labels: List[str] = []
    for src_lvl, tok_level in enumerate(evidence_tokens_levels):
        if not torch.is_tensor(tok_level) or tok_level.dim() != 3 or tok_level.shape[0] <= 0:
            continue
        tok_np = tok_level[0].detach().cpu().numpy()  # [T,C]
        for tok_idx in range(tok_np.shape[0]):
            all_token_vecs.append(tok_np[tok_idx].reshape(-1))
            all_token_labels.append(f"L{src_lvl}T{tok_idx}")

    if len(all_token_vecs) >= 2:
        # 跨 level 维度可能不完全一致，按最短长度对齐
        min_len = min(v.shape[0] for v in all_token_vecs)
        if min_len > 1:
            x = np.stack([v[:min_len] for v in all_token_vecs], axis=0).astype(np.float32)  # [T_all,D]
            corr_mat = np.corrcoef(x)  # [T_all,T_all]
            corr_mat = np.nan_to_num(corr_mat, nan=0.0, posinf=0.0, neginf=0.0)
            corr_mat = np.clip(corr_mat, -1.0, 1.0)

            n_tok = corr_mat.shape[0]
            fig_w = max(7.0, min(16.0, 0.22 * n_tok + 4.0))
            fig_h = max(6.0, min(14.0, 0.22 * n_tok + 3.0))
            plt.figure(figsize=(fig_w, fig_h))
            im = plt.imshow(corr_mat, vmin=-1.0, vmax=1.0, cmap="coolwarm", aspect="auto")
            plt.colorbar(im, fraction=0.046, pad=0.04)
            plt.title(f"{seq} all-level all-token correlation")
            plt.xlabel("token index")
            plt.ylabel("token index")
            if n_tok <= 32:
                ticks = np.arange(n_tok)
                plt.xticks(ticks, all_token_labels, fontsize=7, rotation=90)
                plt.yticks(ticks, all_token_labels, fontsize=7)
            plt.tight_layout()
    ...
    for lvl, spec_evi in enumerate(spectral_evidence_levels):
        if not torch.is_tensor(spec_evi):
            continue
        if spec_evi.dim() != 4:
            continue
        # [B,K,H,W] -> 固定可视化 batch0
...
def main():
    ...
    parser.add_argument(
        "--train-config",
        type=str,
        default="debug_20260414.yaml",
        help=(
            "Train config yaml path. "
            "Absolute path is used directly; "
            "relative path is resolved relative to this script directory."
        ),
    )
    parser.add_argument(
        "--checkpoint",
        ...
        help=(
            "Checkpoint (.pth) path. "
            "Absolute path is used directly; "
            "relative path is resolved under OUTPUTS_DIR in the loaded train config."
        ),
    )
    parser.add_argument(
        "--data-root",
