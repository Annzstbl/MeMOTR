...
def run_forward_once(model: nn.Module,
    ...
    model.eval()
    TrackInstances.set_static_properties(use_spectral_decoder=get_model(model).decoder_spectral, decoder_spectral_weights_dim=get_model(model).decoder_spectral_clusters * 8)
    with torch.no_grad():
...
def save_value_heatmap(arr: np.ndarray, path: str, vmin=None, vmax=None, cmap: str = "jet", fmt: str = ".3f") -> None:
    """
    Heatmap with numeric text on every cell.
    Supports 1D ([N]) and 2D ([H, W]); 1D is reshaped to [1, N].
    """
    arr_np = np.asarray(arr)
    if arr_np.ndim == 1:
        arr2d = arr_np.reshape(1, -1)
    elif arr_np.ndim == 2:
        arr2d = arr_np
    else:
        raise ValueError(f"save_value_heatmap only supports 1D/2D, got shape={arr_np.shape}")
    ...
    h, w = arr2d.shape
    os.makedirs(os.path.dirname(path), exist_ok=True)
    fig_w = max(6.0, min(24.0, 0.7 * w))
    fig_h = max(2.5, min(24.0, 0.7 * h))
    plt.figure(figsize=(fig_w, fig_h))
    im = plt.imshow(arr2d, cmap=cmap, vmin=vmin, vmax=vmax, aspect="auto")
    plt.colorbar(im)

    threshold = (float(vmin) + float(vmax)) * 0.5
    for i in range(h):
        for j in range(w):
            val = float(arr2d[i, j])
            text_color = "white" if val < threshold else "black"
            plt.text(j, i, format(val, fmt), ha="center", va="center", color=text_color, fontsize=8)
...
def save_feature_pca_rgb(t: torch.Tensor, path: str) -> None:
    ...
    U, _, _ = torch.linalg.svd(cov.cpu().float())
...
def main():
    ...
    parser.add_argument("--csv-output-dir", type=str, default="/data/users/wangying01/lth/hsmot/MeMOTR/debug/encoder_tensor_csvs",
    ...
    parser.add_argument("--heatmap-dir", type=str, default="/data/users/wangying01/lth/hsmot/MeMOTR/debug/encoder_tensor_maps",
    ...
    parser.add_argument("--split", type=str, help="Split folder, e.g., val or test.", default='test')
    ...
    # Hook targets
    # parser.add_argument("--hook-modules", type=str, default="scem_module.posterior.pi_head, scem_module.posterior.gate_head, scem_module.posterior.mu_b_head, scem_module.posterior.logsig_b_head,",
                ...
                if isinstance(v, torch.Tensor):
                    ...
                    if k in heatmap_outputs:
                        ...
                        for c in range(min(8, v_np.shape[1])):
                            arr2d = v_np[0, c]
                            save_heatmap(arr2d, os.path.join(map_out_dir, f"{seq}__heatmap_{k}__ch{c}.png"), vmin=v_np.min(), vmax=v_np.max(), add_colorbar=True)
                else:
                    try:
                        desc = str(type(v))
                    except Exception:
            ...
            for k, t in captured.items():
                ...
                if t.dim() == 4 and t.shape[0] == 1:
                    with torch.no_grad():
                        ...
                        vmin = t_np.min()
                        vmax = t_np.max()
                        for c in range(min(8, t.shape[1])):
                            arr2d = t_np[0, c]
                            # CSV (2D)
                            ...
                            if args.save_heatmaps:
                                ...
                                save_heatmap(arr2d, png_path, vmin=vmin, vmax=vmax, add_colorbar=True)
                else:
