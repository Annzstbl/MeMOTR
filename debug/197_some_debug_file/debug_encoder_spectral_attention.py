...
def main():
    ...
    parser.add_argument("--train-config", type=str,
                        ...
                        default='/data/users/litianhao/hsmot_code/MeMOTR/configs_hsmot_spectral_embed_197/10_train_hsmot8ch_spectralembConv_fconv10lr_spectralAttn.yaml')
    parser.add_argument("--checkpoint", type=str, help="Path to .pth checkpoint to load.",
    default='/data4/litianhao/hsmot/memotr/spectralemb/10_encoder_spectral_attn/checkpoint_18.pth'
    )
    ...
    parser.add_argument("--csv-output-dir", type=str, default="/data/users/litianhao/hsmot_code/MeMOTR/debug/encoder_attention/encoder_tensor_csvs",
                        help="Directory to save CSV files (default: ./tensor_csvs).")

    # >>> NEW: heatmap saving options
    parser.add_argument("--save-heatmaps", action="store_true", default=False,
                        help="Whether to save 2D heatmaps of hook outputs (expects 1x8xMxN).")
    parser.add_argument("--heatmap-dir", type=str, default="/data/users/litianhao/hsmot_code/MeMOTR/debug/encoder_attention/encoder_tensor_maps",
    ...
    parser.add_argument("--seq", type=str, help="Sequence folder name to read first frame from.", default='data25-1')
    parser.add_argument("--npy2rgb", action="store_true", help="Whether to convert npy to rgb as in SeqDataset.")

    # Hook targets
    parser.add_argument("--hook-modules", type=str, default="transformer.encoder.layers.0.self_attn.spectral_weights_predictor,transformer.encoder.layers.0.self_attn.sampling_offsets,transformer.encoder.layers.0.self_attn.attention_weights",
