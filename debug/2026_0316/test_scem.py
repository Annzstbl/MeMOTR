...
用法示例:
  python debug/20260316_test_scem/test_scem.py \\
    --train-config /data/users/wangying01/lth/hsmot/MeMOTR/configs_hsmot_spectral_embed_99/20260310_1_from197.yaml \\
    --checkpoint /data4/litianhao/hsmot/memotr/spectralemb/20260310_1/checkpoint_39.pth \\
    --seq data39-1 \\
    --save-csv --save-heatmaps
"""
import os
...
def save_scalar_heatmap(arr2d: np.ndarray, path: str, vmin=None, vmax=None, cmap: str = "jet") -> None:
...
def feature_energy_map(t: torch.Tensor) -> torch.Tensor:
    """
    t: [B,C,H,W] or [C,H,W]
    return: [H,W] 的 energy map
    """
    if t.dim() == 3:
        t = t.unsqueeze(0)
    assert t.dim() == 4
    t0 = t[0]  # [C,H,W]
    energy = torch.sqrt((t0 ** 2).sum(dim=0))
    return energy


def save_feature_energy_heatmap(t: torch.Tensor, path: str) -> None:
    eng = feature_energy_map(t).cpu().numpy()
    vmin, vmax = float(eng.min()), float(eng.max())
    save_scalar_heatmap(eng, path, vmin=vmin, vmax=vmax, cmap="magma")


def save_feature_pca_rgb(t: torch.Tensor, path: str) -> None:
    """
    使用 PCA 将多通道特征压到 3 维并以 RGB 图形式保存。
    只对第一个 batch 做可视化。
    """
    if t.dim() == 3:
        t = t.unsqueeze(0)
    assert t.dim() == 4
    b, c, h, w = t.shape
    x = t[0].permute(1, 2, 0).reshape(-1, c)  # [H*W, C]
    ...
    # 计算协方差矩阵的前 3 个特征向量
    cov = x_centered.t().mm(x_centered) / max(x_centered.shape[0] - 1, 1)
    U, S, Vh = torch.linalg.svd(cov.cpu().float())
    W = U[:, :3].to(x_centered.device)  # [C,3]

    x3 = x_centered.mm(W)  # [H*W,3]
    x3_np = x3.cpu().numpy()
    x3_min = x3_np.min(axis=0, keepdims=True)
    x3_max = x3_np.max(axis=0, keepdims=True)
    x3_np = (x3_np - x3_min) / (x3_max - x3_min + 1e-6)
    img = x3_np.reshape(h, w, 3)
...
def main():
    ...
    parser.add_argument(
        "--train-config",
        type=str,
        default="/data/users/wangying01/lth/hsmot/MeMOTR/configs_hsmot_spectral_embed_99/debug_onePic.yaml",
        help="Path to train config yaml.",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="/data4/litianhao/hsmot/memotr/debug99/checkpoint_39.pth",
    ...
    parser.add_argument(
        "--seq",
        type=str,
        default="data39-1, data48-1, data46-12, data36-13, data30-10",
    ...
    parser.add_argument(
        "--csv-output-dir",
        type=str,
        default="/data/users/wangying01/lth/hsmot/MeMOTR/debug/2026_0316/csvs",
    ...
    parser.add_argument(
        "--heatmap-dir",
        type=str,
        default="/data/users/wangying01/lth/hsmot/MeMOTR/debug/2026_0316/heatmaps",
    ...
    parser.add_argument(
        "--hook-params",
        type=str,
        default="scem_module.posterior.pi_head.spec_pi.spectral_db_logits",
        help="Comma-separated substrings to match parameter/buffer names; matched params are printed and saved as CSV/heatmap.",
    )
    parser.add_argument(
        "--hook-modules",
        type=str,
        # default="scem_module,scem_module.feature_fusion,scem_module.posterior.bg_head,scem_module.posterior.bg_head.mu_b_head,scem_module.posterior.bg_head.logsig_b_head,scem_module.posterior.pi_head,scem_module.posterior.pi_head.aspp,scem_module.posterior.pi_head.spec_proj,scem_module.posterior.pi_head.local_fuse,scem_module.posterior.pi_head.spec_pi, transformer.encoder.layers.0.cross_attention_prior, transformer.encoder.layers.5.cross_attention_prior",
        default="transformer.encoder.layers.0.cross_attention_prior, transformer.encoder.layers.5.cross_attention_prior",
        help="Comma-separated module paths to hook (default includes SCEM feature_fusion / BGHead / PIHead).",
    )
    ...
    if hook_param_patterns:
        ...
        if matched_param_names:
            param_csv_dir = os.path.join(args.csv_output_dir, "params")
            param_map_dir = os.path.join(args.heatmap_dir, "params")
    ...
    for seq in seq_list:
        ...
        if captured:
            ...
            for k, t in captured.items():
                print(f"  {k}: shape={list(t.shape)}, dtype={t.dtype}, device={t.device}")
                ...
                is_mask = (t_det.dtype == torch.bool) or ("mask" in name_lower)
                if is_mask:
                    tt = t_det
                    if tt.dim() == 3:
                    ...
                    if args.save_heatmaps:
                        ...
                        save_scalar_heatmap(arr2d, out_path, vmin=0.0, vmax=1.0, cmap="gray")
                ...
                if "cross_attention_prior" in name_lower and ".out.1" in name_lower:
                    tt = t_det
                    if tt.dim() != 3 or tt.shape[0] != 1 or tt.shape[2] != 8:
                        ...
                        if num_tokens == 24225 and num_ch == 8 and args.save_heatmaps:
                            ...
                            for lvl, (hh, ww) in enumerate(scales):
                                ...
                                save_scalar_heatmap(tiled, out_path, vmin=vmin, vmax=vmax, cmap="jet")
                ...
                # 2) 判别型量：直接 heatmap
                is_discriminative = (
                    "posterior.bg_head.out" in name_lower or          # log_pb
                    "posterior.pi_head.out" in name_lower or          # pi
                    "scem_module.out" in name_lower 
                )
                ...
                if is_discriminative:
                    ...
                    if tt.dim() == 4 and args.save_heatmaps:
                        ...
                        save_scalar_heatmap(arr2d, out_path, vmin=vmin, vmax=vmax, cmap="jet")
                    continue
                  

                # 3) 特征型量：feature energy + PCA
                is_feature_like = any(s in name_lower for s in [
                    "feature_fusion",                  # features/specs 输入与 feat/spec 输出
                    "posterior.bg_head.mu_b_head",     # mu_b
                    "posterior.bg_head.logsig_b_head", # log_sigb
                    "posterior.pi_head.aspp",          # a
                    "posterior.pi_head.spec_proj",     # f_spec
                    "posterior.pi_head.local_fuse",    # h
                    "scem_module", 
                    "posterior.pi_head.spec_pi.out"
                ])

                if is_feature_like and t_det.dim() >= 3 and args.save_heatmaps:
                    tt = t_det
                    if tt.dim() == 3:
                        tt = tt.unsqueeze(0)
                    fe_path = os.path.join(map_out_dir, f"{seq}__{safe_name}__featE.png")
                    save_feature_energy_heatmap(tt, fe_path)
                    pca_path = os.path.join(map_out_dir, f"{seq}__{safe_name}__pca.png")
                    save_feature_pca_rgb(tt, pca_path)
                    continue

                # 4) 其它情况：回退到原有通道逐图 heatmap，方便粗看
                tt = t_det
                if tt.dim() == 3:
                    tt = tt.unsqueeze(1)
                if tt.dim() == 4 and tt.shape[0] == 1 and args.save_heatmaps:
                    t_np = tt.cpu().numpy()
                    vmin, vmax = t_np.min(), t_np.max()
                    for c in range(0, min(16, tt.shape[1]), 2):
                        arr2d = t_np[0, c]
                        db.save_heatmap(
                            arr2d,
                            os.path.join(map_out_dir, f"{seq}__{safe_name}__ch{c}.png"),
                            vmin=vmin,
                            vmax=vmax,
                            add_colorbar=True,
                        )
