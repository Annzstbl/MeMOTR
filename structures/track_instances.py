...
class TrackInstances:
    ...
    def __init__(self, frame_height: float = 1.0, frame_width: float = 1.0,
                 hidden_dim: int = 256, num_classes: int = 1, use_dab: bool = False, use_spectral_decoder: bool = False, decoder_spectral_weights_dim: int = 8):
        ...
        self.decoder_spectral_weights_dim = decoder_spectral_weights_dim
        if self.use_dab:
        ...
        self.query_spectral_weights = torch.zeros((0, decoder_spectral_weights_dim), dtype=torch.float)# 如果参与预测，使用的query。不一定是最后预测的光谱权重，可能没更新。
        self.pred_spectral_weights = torch.zeros((0, decoder_spectral_weights_dim), dtype=torch.float)# 最后预测的光谱权重
    ...
    # 只在初始化GT时候使用
    @staticmethod
    def init_tracks(batch: dict, hidden_dim: int, num_classes: int, device="cpu", use_dab: bool = False, decoder_spectral_weights_dim: int = 8):
        """
